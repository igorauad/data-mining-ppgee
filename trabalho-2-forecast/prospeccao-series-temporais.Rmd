...
```{r, include=FALSE}
tsDiario2mth <- window(tsDiario, start=c(2008,2), end=c(2008,61))
```
...
### Fluxo Diário para um intervalo arbitrário de 2 meses
```{r, echo=FALSE}
...
```{r, cache=TRUE}
tsDiarioTrain <- window(tsDiario, end=c(2007,365)) # De Jan 2002 a Dez 2005
```
...
##### 30 dias:
```{r, cache=TRUE}
tsDiarioTest30Days <- window(tsDiario, start=c(2008,1), end=c(2008,30)) # De 1/1/2006 a 30/1/2006
```

##### 45 dias:
```{r, cache=TRUE}
# 45 dias a partir De 1/1/2006
tsDiarioTest45Days <- window(tsDiario, start=c(2008,1), end=c(2008,45)) 
```
...
```{r, include=FALSE}
tsDiarioTrainMszl2mth <- msts(tsDiario2mth, seasonal.periods=c(7,365.25))
```
...
#### Decomposição do fluxo diário em termos trend+season para um intervalo arbitrário de 2 meses
```{r, cache=TRUE}
tbats_model2mth <- tbats(tsDiarioTrainMszl2mth)
```

```{r}
plot(tbats_model2mth)
```
